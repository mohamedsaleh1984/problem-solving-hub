...
{
public:
       vector<int> rowVector(vector<vector<int>>& matrix, int iRow)
    {
        vector<int> newVec = matrix[iRow];
        sort(newVec.begin(), newVec.end());
        return newVec;
    }

    vector<int> colVector(vector<vector<int>>& matrix, int iCol)
    {
        vector<int> newVec = {};
        for (int iRow = 0; iRow < matrix.size(); iRow++)
            newVec.push_back(matrix[iRow][iCol]);
        sort(newVec.begin(), newVec.end());
        return newVec;
    }

    vector<int> Ref(int size)
    {
        vector<int> newVec;
    ...
    {
        int n = matrix.size();
        vector<int> ref = Ref(n);

        for (int i = 0; i < matrix.size(); i++)
        {
            if (rowVector(matrix,i) != ref || colVector(matrix,i) != ref)
            {
                return false;
            }
        }
...
int main(void)
{
    ...
    cout << (s.checkValid(vec) ? "true" : "false") << endl;
